...
def get_ps_output():
    try:
        ...
        process = subprocess.Popen(["lsof", "-RK"], stdout=subprocess.PIPE)
...
if ps_output:
    ...
    for line in lines:
        ...
        if len(parts) >= 12:
            print("pr")
            cmd, pid, tid, taskcmd, ppid, user, fd, typ, device, size_off, node = parts[:11]
            comm_list = parts[11:]
            ...
            if str(pid) != str(ps_pid)  and str(ppid) != str(ps_pid) and str(tid) != str(ps_pid) and str(pid) != str(current_pid) and str(ppid) != str(current_pid) and tid != current_pid:
                ...
                process_info[tid] = {'ppid': ppid, 'pid':pid, 'taskcmd':taskcmd, 'user': user, 'comm': comm}
    ...
    for tid, info in process_info.items():
        ...
        parent_node_label = f"{info['pid']} {info['user']}\n{info['taskcmd']}\n{info['comm']}"
        dot.node(info['pid'], parent_node_label)
    ...
    try:
        ...
        dot.render('lsof_graph', format='png', view=False)
        dot.render('lsof_graph', format='jpg', view=False)
        dot.render('lsof_graph', format='pdf', view=False)
